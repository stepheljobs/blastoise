...
io.on('connection', function (socket) {
	...
	socket.on('viewUserNotifications', function (notif) {
		connection.query('select notif.id, notif_message.created_at, notif.user_id, notified_by, type, message, viewed from notifications notif left join notification_messages notif_message on notif_message.id = notif.notification_message_id where notif.user_id = ? order by notif_message.created_at desc limit ?, ? ', [notif.user_id, notif.record_start, notif.number_of_records], function (error, result) {
	...
	socket.on('viewNotification', function (notif) {
		connection.query('select notif.id, notif_message.created_at, notif.user_id, notified_by, type, message, viewed from notifications notif left join notification_messages notif_message on notif_message.id = notif.notification_message_id where notif.user_id = ? and notif.id = ?', [notif.user_id, notif.id], function (error, result) {
			var isValid = false;
			var notifLink = "";
	...
	function saveNotification(userNotif){
		connection.query('insert into notifications set ?', userNotif , function (error, result) {
			...
			connection.query('select notif.id, notif_message.created_at, notif.user_id, notified_by, type, message, viewed from notifications notif left join notification_messages notif_message on notif_message.id = notif.notification_message_id where notif.user_id = ? and notif.id = ?', [userNotif.user_id, result.insertId], function (_error, newlyInsertedNotification) {
